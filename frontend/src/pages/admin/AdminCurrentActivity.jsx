  ...
  const fetchData = async (isRefresh = false) => {
    ...
    try {
      const [sessionData, notifs] = await Promise.all([
        adminService.getTeamCurrentActivity(),
      ...
      <main className="dashboard-main">
        <div className="manager-dashboard-container">
          <header className="dashboard-header">
            <div>
              <h1 className="dashboard-title">Organization Activity</h1>
              <p className="dashboard-subtitle">Real-time status of all working employees</p>
            </div>
          ...
          <div className="manager-overview-panel" style={{ padding: '20px' }}>
            <div style={{ borderBottom: '1px solid #e2e8f0', paddingBottom: '16px', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}>
               <Activity size={18} style={{ color: '#2563eb' }} />
               <h3 className="manager-overview-heading" style={{ margin: 0, border: 'none', padding: 0 }}>Active & Idle Sessions</h3>
            </div>

            {loading ? (
              <div className="dash-loading" style={{ padding: '40px' }}>Loading activity...</div>
            ) : sessions.length === 0 ? (
              <div style={{ padding: '60px 20px', textAlign: 'center', color: '#64748b' }}>
                ...
                <p style={{ margin: 0, fontSize: '14px' }}>No employees are currently working or idle.</p>
              </div>
            ) : (
...
export default AdminCurrentActivity;
